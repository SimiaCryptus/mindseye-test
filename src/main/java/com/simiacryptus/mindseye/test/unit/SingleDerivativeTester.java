...
public class SingleDerivativeTester extends ComponentTestBase<ToleranceStatistics> {
  ...
  public ToleranceStatistics test(@Nonnull final NotebookOutput output, @Nonnull final Layer component,
    ...
    SimpleEval temp_00_0023 = SimpleEval.run(component == null ? null : component.addRef(),
    ...
    if (verbose) {
      output.run(RefUtil.wrapInterface(() -> {
        log.info(RefString.format("Inputs: %s", RefArrays.stream(Tensor.addRefs(inputPrototype)).map(t -> {
          String temp_00_0003 = t.prettyPrint();
          if (null != t)
            t.freeRef();
          return temp_00_0003;
        }).reduce((a, b) -> a + ",\n" + b).orElse("")));
        log.info(RefString.format("Inputs Statistics: %s", RefArrays.stream(Tensor.addRefs(inputPrototype)).map(x -> {
      ...
      }, outputPrototype == null ? null : outputPrototype.addRef(), Tensor.addRefs(inputPrototype)));
    }
    if (isTestFeedback()) {
      ...
      _statistics = output.eval(RefUtil.wrapInterface(() -> {
        return testFeedback(statistics, component == null ? null : component.addRef(), Tensor.addRefs(inputPrototype),
    ...
    if (isTestLearning()) {
      ...
      final ToleranceStatistics statistics = _statistics;
      _statistics = output.eval(RefUtil.wrapInterface(() -> {
        return testLearning(statistics, component == null ? null : component.addRef(), Tensor.addRefs(inputPrototype),
            outputPrototype == null ? null : outputPrototype.addRef());
      }, outputPrototype == null ? null : outputPrototype.addRef(), Tensor.addRefs(inputPrototype),
          component == null ? null : component.addRef()));
    }
    ...
    output.run(RefUtil.wrapInterface(() -> {
      testFrozen(component == null ? null : component.addRef(), Tensor.addRefs(inputPrototype));
      testUnFrozen(component == null ? null : component.addRef(), Tensor.addRefs(inputPrototype));
  ...
  public ToleranceStatistics testLearning(@Nonnull ToleranceStatistics prev, @Nonnull Layer component,
    ...
    ToleranceStatistics temp_00_0020 = RefIntStream.range(0, temp_00_0024.size())
        .mapToObj(RefUtil.wrapInterface((IntFunction<ToleranceStatistics>) i -> {
          Tensor temp_00_0025 = measureLearningGradient(component == null ? null : component.addRef(), i,
              outputPrototype == null ? null : outputPrototype.addRef(), Tensor.addRefs(inputPrototype));
          @Nullable
          final Tensor measuredGradient = !verify ? null : temp_00_0025.addRef();
          if (null != temp_00_0025)
            temp_00_0025.freeRef();
          @Nonnull
          final Tensor implementedGradient = getLearningGradient(component == null ? null : component.addRef(), i,
              outputPrototype == null ? null : outputPrototype.addRef(), Tensor.addRefs(inputPrototype));
          @Nonnull
          Tensor difference = measuredGradient.minus(implementedGradient == null ? null : implementedGradient.addRef());
          try {
  ...
  public ToleranceStatistics testFeedback(@Nonnull ToleranceStatistics statistics, @Nonnull Layer component,
    ...
    Optional<ToleranceStatistics> optional = RefIntStream.range(0, inputPrototype.length)
        .mapToObj(RefUtil.wrapInterface((IntFunction<ToleranceStatistics>) i -> {
          Tensor temp_00_0027 = measureFeedbackGradient(component == null ? null : component.addRef(), i,
              outputPrototype == null ? null : outputPrototype.addRef(), Tensor.addRefs(inputPrototype));
          @Nullable
          final Tensor measuredGradient = !verify ? null : temp_00_0027.addRef();
          if (null != temp_00_0027)
            temp_00_0027.freeRef();
          @Nonnull
          final Tensor implementedGradient = getFeedbackGradient(component == null ? null : component.addRef(), i,
              outputPrototype == null ? null : outputPrototype.addRef(), Tensor.addRefs(inputPrototype));
  ...
  public void testFrozen(@Nonnull final Layer component, @Nonnull Tensor[] inputPrototype) {
    ...
    @Nonnull
    final Layer frozen = temp_00_0028.freeze();
    if (null != temp_00_0028)
      temp_00_0028.freeRef();
    RefList<TensorArray> inputCopies = RefArrays.stream(Tensor.addRefs(inputPrototype)).map(data -> {
  ...
  public void testUnFrozen(@Nonnull final Layer component, Tensor[] inputPrototype) {
    ...
    @Nonnull
    final Layer frozen = temp_00_0033.setFrozen(false);
    ...
    if (null != eval)
      eval.freeRef();
    @Nullable
    final RefList<double[]> stateList = frozen.state();
    frozen.freeRef();
  ...
  protected void measureFeedback(@Nonnull Layer component, int inputIndex, Tensor baseOutput,
    ...
    @Nonnull
    final Tensor[] copyInput = RefArrays.copyOf(Tensor.addRefs(inputPrototype), inputPrototype.length);
    ReferenceCounting.freeRefs(inputPrototype);
    ...
    inputProbe.freeRef();
    Result[] input1 = ConstantResult.batchResultArray(new Tensor[][] { Tensor.addRefs(copyInput) });
    ReferenceCounting.freeRefs(copyInput);
    try {
      Result temp_00_0036 = component.eval(Result.addRefs(input1));
      TensorList temp_00_0037 = temp_00_0036.getData();
      @Nullable
      final Tensor evalProbe = temp_00_0037.get(0);
      if (null != temp_00_0037)
        temp_00_0037.freeRef();
      if (null != temp_00_0036)
        temp_00_0036.freeRef();
      Tensor temp_00_0038 = evalProbe.minus(baseOutput == null ? null : baseOutput.addRef());
      @Nonnull
      final Tensor delta = temp_00_0038.scaleInPlace(1. / probeSize);
  ...
  private Tensor getFeedbackGradient(@Nonnull final Layer component, final int inputIndex,
    ...
    for (int j = 0; j < outputPrototype.length(); j++) {
      ...
      final Result[] copyInput = RefArrays.stream(Tensor.addRefs(inputPrototype)).map(x -> {
        try {
      ...
      Result temp_00_0002 = new Result(new TensorArray(inputTensor == null ? null : inputTensor.addRef()),
  ...
  private Tensor getLearningGradient(@Nonnull final Layer component, final int layerNum,
    ...
    for (int j = 0; j < outputPrototype.length(); j++) {
      ...
      @Nonnull
      final DeltaSet<UUID> buffer = new DeltaSet<UUID>();
      Result[] array = ConstantResult.batchResultArray(new Tensor[][] { Tensor.addRefs(inputPrototype) });
      @Nullable
      final Result eval = component.eval(Result.addRefs(array));
      for (@Nonnull
      Result result : array) {
        RefUtil.freeRef(result.getData());
      }
  ...
  private Tensor measureFeedbackGradient(@Nonnull final Layer component, final int inputIndex,
    ...
    Result[] input0 = ConstantResult.batchResultArray(new Tensor[][] { Tensor.addRefs(inputPrototype) });
    Result temp_00_0043 = component.eval(Result.addRefs(input0));
    TensorList temp_00_0044 = temp_00_0043.getData();
    @Nullable
    final Tensor baseOutput = temp_00_0044.get(0);
    ...
    for (int probeIndex = 0; probeIndex < inputPrototype[inputIndex].length(); probeIndex++) {
      measureFeedback(component == null ? null : component.addRef(), inputIndex,
          baseOutput == null ? null : baseOutput.addRef(), Tensor.addRefs(inputPrototype),
  ...
  private Tensor measureLearningGradient(@Nonnull final Layer component, final int layerNum,
    ...
    outputPrototype.freeRef();
    Result[] input2 = ConstantResult.batchResultArray(new Tensor[][] { Tensor.addRefs(inputPrototype) });
    if (null != inputPrototype)
      ReferenceCounting.freeRefs(inputPrototype);
    Result temp_00_0046 = component.eval(Result.addRefs(input2));
    TensorList temp_00_0047 = temp_00_0046.getData();
    @Nullable
    final Tensor baseOutput = temp_00_0047.get(0);
    ...
    for (int i = 0; i < stateLen; i++) {
      ...
      Result temp_00_0049 = copy.eval(Result.addRefs(input2));
      TensorList temp_00_0050 = temp_00_0049.getData();
      @Nullable
      final Tensor evalProbe = temp_00_0050.get(0);
      if (null != temp_00_0050)
        temp_00_0050.freeRef();
      if (null != temp_00_0049)
        temp_00_0049.freeRef();
      copy.freeRef();
      Tensor temp_00_0051 = evalProbe.minus(baseOutput == null ? null : baseOutput.addRef());
      @Nonnull
      final Tensor delta = temp_00_0051.scaleInPlace(1. / probeSize);
